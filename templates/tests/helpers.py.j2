...
class TestContext:
    ...
    def skip(self, msg: str) -> None:
        """Log a skipped test (prerequisite state missing — does NOT count as fail)."""
        self.skip_count += 1
        print(f"  ⏭️   SKIP  {msg}")
    ...
    def req(
        ...
        try:
            return requests.request(
                method, url, headers=req_headers, json=body, params=params, timeout=10
            )
    ...
    @staticmethod
    def no_sensitive_field_in(obj: Any, field_name = "password", context: str = "response") -> bool:
        """Recursively verify a sensitive field does not appear in any value.

        field_name may be a string or a list of strings (all must be absent).
        """
        if isinstance(field_name, list):
            return all(TestContext.no_sensitive_field_in(obj, f, context) for f in field_name)
        if isinstance(obj, dict):
            if field_name in obj:
                return False
            return all(TestContext.no_sensitive_field_in(v, field_name, context) for v in obj.values())
        if isinstance(obj, list):
            return all(TestContext.no_sensitive_field_in(item, field_name, context) for item in obj)
        return True
