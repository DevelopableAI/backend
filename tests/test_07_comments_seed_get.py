...
def run(ctx: TestContext) -> None:
    ...
    if post1_id and token1:
        resp = ctx.req("POST", "/api/comments", token=token1, body={
            "body": "Great article, Alice!", "postId": post1_id,
        })
        if ctx.assert_status(resp, 201, "Seed: Create comment1 by user1 on post1"):
            data = ctx.safe_json(resp)
            ctx.state["comment1_id"] = data.get("id")
            if data.get("authorId") == user1_id:
                ctx.ok("authorId correctly injected from token on POST /api/comments")
            else:
    ...
    if post1_id and token2:
        resp = ctx.req("POST", "/api/comments", token=token2, body={
            "body": "Interesting perspective!", "postId": post1_id,
        })
        if ctx.assert_status(resp, 201, "Seed: Create comment2 by user2 on post1"):
            ctx.state["comment2_id"] = ctx.safe_json(resp).get("id")
